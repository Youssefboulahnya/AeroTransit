...
const FlightPassengers = () => {
  ...
  const updateField = (index, field, value) => {
    ...
    // <-- CORRECTION MINIMALE : sanitize seat input to digits only
    if (field === "seat") {
      updated[index][field] = String(value).replace(/\D/g, "");//empecher de faire des caractere non number
    } else {
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      const passengersPayload = forms.map((p) => ({
        ...
        Numero_place: p.seat ? parseInt(p.seat, 10) : null,
      }));
          ...
          <div className="passenger-card" key={i}>
            ...
            <div className="form-row">
              ...
              <div className="form-group">
                ...
                {/* <-- CORRECTION MINIMALE : use text + inputMode to avoid browser auto increment/decrement */}
                <input
                  type="text"
                  inputMode="numeric"
                  value={p.seat}
